...
In the example earlier in map closing, if instead the max map size is 12, we do not assign X as the parent rom direction as that would close the map 1 cell prematurely. BUt what we can do is remove the rooms with 3 or more branches to ensure that the next iteration can only spawn a max of 1 other cell to complete the 12 room map size.
``` 
---2C--   
--2E9--  
---3C--  
